...
try:
    ...
    print("Error: 'weasyprint' is required to generate PDFs.")
    sys.exit(1)
...
def generate_report(json_file_path, format_id, output_path):
    ...
    try:
        ...
        HTML(string=rendered_html, base_url=TEMPLATES_DIR).write_pdf(output_path)
        ...
        return False
def generate_report_from_dict(data: dict, format_id: str, output_path: str):
    """
    Functional entry point for API orchestration. Accepts a dictionary directly.
    """
    formats = load_formats()
    if format_id not in formats:
        print(f"Error: Format ID '{format_id}' not found.")
        return False
    ...
    try:
        ...
        # Also save MD version
        md_output_path = os.path.splitext(output_path)[0] + ".md"
        markdown_text = md(rendered_html, heading_style="ATX", autolinks=False)
        with open(md_output_path, "w", encoding="utf-8") as f:
            f.write(markdown_text)
            
        return True, output_path, md_output_path
    except Exception as e:
        import traceback
        print(f"Manual fail during PDF/MD generation: {e}")
        traceback.print_exc()
