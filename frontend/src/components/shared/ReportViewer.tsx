...
export default function ReportViewer({
    ...
    // Consume version props to trigger re-render when stores update
    void pdfVersion;
    void mdVersion;
    ...
    if (!report && !hasGeneratedPdf && !hasGeneratedMd) {
        return (
            <div className="flex flex-col items-center justify-center h-full p-8">
                ...
                <p className="text-sm mt-1 text-slate-400 dark:text-slate-500 text-center max-w-xs">
                    Generate a report from Clinical Notes to see it here.
                </p>
            </div>
        );
    }

    const title = hasGeneratedPdf
    ...
    return (
        <div className="flex flex-col h-full overflow-auto">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">
                            {totalReports} report{totalReports !== 1 ? 's' : ''} available
                        </p>
                    </div>
            ...
            <div className="px-6 pb-6 space-y-3">
                ...
                {hasGeneratedMd && (
                    <button onClick={() => { setViewMode('md'); setIsOpen(true); }} className="w-full text-left group">
                    ...
                    >
                        <div className={`flex items-center gap-4 p-4 rounded-xl border transition-all duration-200 hover:-translate-y-px ${hasGeneratedPdf || hasGeneratedMd
                            ...
                            <div className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 ${hasGeneratedPdf || hasGeneratedMd ? 'bg-slate-200 dark:bg-slate-700' : 'bg-gradient-to-br from-teal-500 to-emerald-500 shadow-md shadow-teal-500/25'
                                ...
                                <FileText className={`w-5 h-5 ${hasGeneratedPdf || hasGeneratedMd ? 'text-slate-400 dark:text-slate-500' : 'text-white'}`} />
                            </div>
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                    <h3 className={`text-sm font-bold truncate ${hasGeneratedPdf || hasGeneratedMd ? 'text-slate-500 dark:text-slate-400' : 'text-slate-800 dark:text-white'}`}>
                                        {report.title}
                                    </h3>
                                    <span className={`text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded shrink-0 ${hasGeneratedPdf || hasGeneratedMd ? 'bg-slate-200 dark:bg-slate-700 text-slate-400 dark:text-slate-500' : 'bg-teal-500 text-white'
                                        }`}>
                                        Original
                                    </span>
                                </div>
                            ...
                            <ChevronRight className={`w-4 h-4 group-hover:translate-x-0.5 transition-transform shrink-0 ${hasGeneratedPdf || hasGeneratedMd ? 'text-slate-300 dark:text-slate-600' : 'text-teal-400 dark:text-teal-500'
                                }`} />
                        </div>
                    </button>
                )}
                    ...
                    <DialogHeader className="px-5 py-3 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 shrink-0">
                        <DialogTitle className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pr-6">
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                                {hasGeneratedPdf && hasGeneratedMd && (
                                    <div className="relative flex bg-slate-100 dark:bg-slate-800 rounded-lg p-0.5 border border-slate-200 dark:border-slate-700">
                                        <div className={`absolute top-0.5 bottom-0.5 rounded-md shadow-sm transition-all duration-300 ease-in-out ${viewMode === 'pdf' ? 'left-0.5 w-[calc(50%-2px)] bg-sky-500' : 'left-[calc(50%+2px)] w-[calc(50%-4px)] bg-violet-500'
                                            }`} />
                                        <button onClick={() => setViewMode('pdf')} className={`relative z-10 px-3 py-1 text-xs font-bold rounded-md transition-colors duration-200 ${viewMode === 'pdf' ? 'text-white' : 'text-slate-400 hover:text-slate-600'}`}>PDF</button>
                                        <button onClick={() => setViewMode('md')} className={`relative z-10 px-3 py-1 text-xs font-bold rounded-md transition-colors duration-200 ${viewMode === 'md' ? 'text-white' : 'text-slate-400 hover:text-slate-600'}`}>Markdown</button>
                                    </div>
                                )}
                                {viewMode === 'pdf' ? (
                                    <>
                                    ...
                                    <Button size="sm" variant="outline" className="gap-1.5 h-7 text-xs" onClick={() => {
                                        const blob = new Blob([generatedMd || appointmentReport], { type: 'text/markdown' });
